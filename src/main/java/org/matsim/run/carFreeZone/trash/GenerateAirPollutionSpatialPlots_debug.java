package org.matsim.run.carFreeZone.trash;
...
public class GenerateAirPollutionSpatialPlots_debug {
    private static final Logger log = Logger.getLogger(GenerateAirPollutionSpatialPlots_debug.class);
...
/*    private static final double xMin = 4588458.659;
    ...
    //bigger BoundingBox:
    private static final double xMin = 4577849;
    private static final double yMin = 5822130.;
    private static final double xMax = 4579891.;
    private static final double yMax = 5823713.;
    // --- haowu ---
    ...
    private GenerateAirPollutionSpatialPlots_debug() {

    }

    public static void main(String[] args) {

        GenerateAirPollutionSpatialPlots_debug plots = new GenerateAirPollutionSpatialPlots_debug();
    ...
    private void writeEmissions() {
        ...
        final String outputFile = outputDir + runId + ".emissionsgrid_boundingbox_debug.csv";
            ...
            for (Grid.Cell<Map<Pollutant, Double>> cell : bin.getValue().getCells()) {
                for (var p : pollutants) {
                    if (cell.getValue().containsKey(p)) {
                        if(cell.getValue().get(p).isNaN()) {
                            System.out.println(cell.getValue().get(p));
                        }
                    } else {
